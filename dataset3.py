...
def processData(numSamples, nv, decimals, 
                ...
                xRange=[0.1,3.1], testPoints=True, # TODO activate to get XT and YT
    ...
    for i in tqdm(range(numSamples)):
        ...
        try:
            skeletonEqn = "exp(x1)" # TODO hard-coded for now
        ...
        for e in range(numSamplesEachEq):
            ...
            #TODO added this myself
            if testPoints:
                # generate test data points
                nTestPoints = np.random.randint(
                    *numberofPoints) if supportPointsTest is None else len(supportPointsTest)

                try:
                    dataTest = generateDataStrEq(cleanEqn, n_points=nTestPoints, n_vars=nv, decimals=decimals,
                                                supportPoints=supportPointsTest, min_x=testRange[0], max_x=testRange[1])
                    XT, YT = dataTest
                except:
                    # handle exceptions
            ...
            structure['Y'] = y
            if testPoints:
                structure['XT'] = list(XT)  # Add test X data
                structure['YT'] = YT        # Add test Y data
...
def main():
   # Config
    ...
    numSamples = 100  # number of generated samples
    folder = './Dataset'
