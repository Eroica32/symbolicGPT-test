...
def main(resultDict, modelKey):
 ...
 # TODO: change everything to the same, change dir to the correct ones, everything same to the 
    ...
    dataFolder = 'datasets/exponential' #3Var_RandSupport_FixedLength_-3to3_-5.0to-3.0-3.0to5.0_500Points'
    dataTestFolder = 'datasets/exponential/Test' #3Var_RandSupport_FixedLength_-3to3_-5.0to-3.0-3.0to5.0_500Points/Test'
    ...
    fName = '{}_SymbolicGPT_{}_{}_{}_MINIMIZE'.format(dataInfo, 
                                                'GPT_PT_{}_{}'.format(method, target), 
                                                'Padding',
                                                variableEmbedding)
    ckptPath = '{}/{}.pt'.format(addr,fName.split('.pt')[0]) # TODO changed from txt to pt
    try: 
        os.mkdir(addr)
    except:
    ...
    train_file = fName[:-4] + '.pt'  # TODO changed
    ...
    if os.path.isfile(train_file) and not scratch:
        ...
        chars = sorted(list(set(text))+['_','T','<','>',':']+['1', '2', '3', '4', '5', '6', '7', '8']) # extract unique characters from the text before converting the text to a list, # T is for the test data
        print(chars)
    ...
    pconf = PointNetConfig(embeddingSize=512,
                       numberofPoints=49,
                       numberofVars=numVars, 
                       numberofYs=numYs,
                       method=method,
